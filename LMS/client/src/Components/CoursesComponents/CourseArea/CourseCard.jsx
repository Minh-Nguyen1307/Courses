...
const CourseCard = ({ course }) => {
    ...
    const addToCart = async (userId, courseId) => {
        try {
            // Make the API request with the courseId
            const response = await axios.post(
                `${import.meta.env.VITE_API_BASE_URL}/users/${userId}/add`,
                { courseId }  // Send the courseId in the request body
            );
            return response.data; // Return the response data (success or failure)
        } catch (error) {
            // Handle errors, including course already in cart scenario
    ...
    const handleAddToCart = async () => {
        ...
        try {
            // Call addToCart with userId and courseId
