  ...
  try {
    ...
    const existingCartItem = user.cart.find((cartItem) => {
      if (!cartItem.courseId) {
        console.error("Cart item missing courseId", cartItem); // Log missing courseId
        return false; // Skip this item if courseId is missing
      }
      return cartItem.courseId.toString() === course._id.toString(); // Compare courseId
    });
    ...
    // Return a success response with the updated cart and total price
    res.status(200).json({
